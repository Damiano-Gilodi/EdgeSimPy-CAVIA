...
Updated to include full-duplex links
...
def partially_connected_fullduplex_hexagonal_mesh(network_nodes: list, link_specifications: list) -> Topology:
    ...
    for node in network_nodes:
        ...
        for neighbor_coordinates in neighbors:
            ...
            if (neighbor, node) in physical_links:
                continue

            physical_links.append((node, neighbor))

            if not topology.has_edge(node, neighbor):
                # NetworkX edge creation (needed for adjacency structure)
                topology.add_edge(node, neighbor)

            # Creating network link forward object
    ...
    if links_with_missing_specs:
        raise Exception(f"You must specify the properties for {len(physical_links)} links or ignore the 'link_specifications' parameter.")

    # Applying the user-specified attributes to the network links
    links = (link for link in random.sample(physical_links, len(physical_links)))
    for spec in link_specifications:
        for _ in range(spec["number_of_objects"]):
            (u, v) = next(links)

            link_f = topology._adj[u][v]
            link_b = topology._adj[v][u]
